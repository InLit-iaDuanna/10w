...
Materialize, check, build, then explicitly open the new Demo. Existing custom game code may need a separately scoped loader edit to use createDemoAsset, setDemoDoorOpen and demoAssetBlocksPlayer. A GLB export alone is not game integration. Verify appearance, hinge, collision and existing key/distance/angle independently. Claim visual understanding only when an image was actually provided to the model.
