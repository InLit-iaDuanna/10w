...
class CodeBuddyFailureTests(unittest.IsolatedAsyncioTestCase):
    async def test_invalid_output_does_not_become_suggestion(self):
        ...
        with patch('sceneops_design_ai.codebuddy.model_catalog', new=AsyncMock(return_value=ModelCatalog(mode='live', models=['hy3'], message='fixture'))), patch('sceneops_design_ai.codebuddy.invoke_cli', new=AsyncMock(return_value='not json')):
