...
export function LogicWorkbench(props: LogicWorkbenchProps) {
  const query = useQuery({ queryKey: logicWorkbenchKeys.demo, queryFn: props.api.loadDemo, retry: false, refetchOnWindowFocus: false });
  const [draft, setDraft] = useState<GameplayGraph | null>(null);
  const graph = draft ?? query.data;
  const [selectedNode, setSelectedNode] = useState('collectible_pickup');
  const [tab, setTab] = useState<'graph' | 'relationships' | 'json' | 'code'>('graph');
  const [jsonDraft, setJsonDraft] = useState('');
  const [rationale, setRationale] = useState('明确钥匙拾取与家门开启的交互意图');
  const [codeDiff, setCodeDiff] = useState('');
  const [codePath, setCodePath] = useState('Assets/SceneOps/Homecoming.cs');
  ...
  const status = query.isPending ? 'loading' : query.isError ? 'failed' : graph ? 'ready' : 'empty';
  ...
  function update(value: GameplayGraph) {
    ...
    setDraft({ ...value, version: query.data.version + 1 });
  ...
  return <section className="logic-workbench"><div className="panel-header"><span>02 / 玩法逻辑</span><span className="badge mock">{screen?.modeLabel ?? 'MOCK'} · {screen?.statusLabel ?? '加载中'}</span></div>
    {query.isPending && <p className="notice">正在连接本地玩法 API…</p>}
    ...
    {graph && <><fieldset disabled={busy} className="logic-fields">
      ...
      {tab === 'json' && <div className="json-editor"><p>完整编辑状态变量、节点、条件、效果和边；更新前由同一个确定性服务检查。</p><textarea aria-label="完整玩法图 JSON" value={jsonDraft} onChange={event => setJsonDraft(event.target.value)}/><button onClick={applyJson}>验证并更新草稿</button></div>}
      {tab === 'code' && <div className="json-editor"><p>粘贴 unified diff 生成既有 CodeChangeSet。此入口不执行 C#、编译或写文件。</p><label>Unity 相对路径<input value={codePath} onChange={event => setCodePath(event.target.value)}/></label><textarea aria-label="C# unified diff" placeholder={'--- a/Assets/SceneOps/Homecoming.cs\n+++ b/Assets/SceneOps/Homecoming.cs\n@@ …'} value={codeDiff} onChange={event => setCodeDiff(event.target.value)}/><button onClick={proposeCode}>生成代码 ChangeSet</button></div>}
