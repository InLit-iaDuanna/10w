...
class CodeBuddyTests(unittest.IsolatedAsyncioTestCase):
    async def test_text_only_arguments_and_success(self):
        ...
        with patch('conversation_home.codebuddy.shutil.which', return_value='/host/codebuddy'), patch('conversation_home.codebuddy.asyncio.create_subprocess_exec', return_value=process) as spawn:
    ...
    async def test_missing_cli_is_blocked(self):
        with patch('conversation_home.codebuddy.shutil.which', return_value=None):
