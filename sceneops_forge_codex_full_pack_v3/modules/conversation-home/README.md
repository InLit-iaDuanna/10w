...
公开后端 `create_ai_router(database_path, project_exists=callback)` 组合：

- `GET /api/ai/models`：CLI 默认模型及统一 15 模型目录，只检查安装。
- `GET/PUT /api/ai/settings`：全局模型设置 `{model}`，SQLite 持久化。
...
统一及旧 API 都复用 `sceneops_codebuddy`，模型调用详情见 integrations/codebuddy-cli/README.md。
...
网络合同：运行 `backend/export_unified_contracts.py` 导出 `contracts/unified-ai.openapi.json`，
